...
\begin{itemize}
    \item Clustering es una técnica de aprendizaje no supervisado.
...
\begin{center}
\includegraphics[scale=0.3]{clase15-customer-segmentation.png}
\end{center}
...
\begin{frame}
\frametitle{Clustering y datos cercanos}
...
\begin{center}
\includegraphics[scale=0.35]{clase15-penguins-clusters.png}
\end{center}
...
\begin{center}
\includegraphics[scale=0.15]{clase15-kmeans-centroid.png}
\end{center}
...
\begin{frame}
\frametitle{Idea del funcionamiento de $k$-medias}

\begin{itemize}
\item Si fijamos unos centros para los clusters, podemos determinar los clusters tomando los puntos más cercanos a cada centro.
\item Si fijamos los clusters, podemos determinar los centros tomando el promedio de todos los puntos del cluster.
\end{itemize}
...
\begin{frame}
\frametitle{Algoritmo $k$-medias (naive)}
...
\begin{enumerate}
\item Actualizamos las etiquetas: para cada punto (dato), buscamos cuál es el centro más cercano a ese punto y lo asignamos a ese cluster.
\item Actualizamos los centros: para cada cluster, recalculamos el centro del cluster como el promedio de todo los datos del cluster (si los puntos tienen varias coordenadas, calculamos el promedio de cada coordenada).
\end{enumerate}

¿Qué significa en este caso alcanzar convergencia? \pause
...
\begin{minipage}{.5\textwidth}
    \begin{center}
    \includegraphics[scale=0.35]{clase15-convexblobs.png}
    \end{center}
\end{minipage}%
\begin{minipage}{.5\textwidth}
    \begin{center}
    \includegraphics[scale=0.35]{clase15-nonconvexblobs.png}
    \end{center}
\end{minipage}
...
\begin{minipage}{.5\textwidth}
    \begin{center}
    \includegraphics[scale=0.08]{clash-fingerUp.png}
    
    \includegraphics[scale=0.35]{clase15-convexblobs-etiquetado.png}
    \end{center}
\end{minipage}%
\begin{minipage}{.5\textwidth}
    \begin{center}
    \includegraphics[scale=0.08]{clash-sad.png}
    
    \includegraphics[scale=0.35]{clase15-nonconvexblobs-etiquetado.png}
    \end{center}
\end{minipage}
...
\begin{frame}
\frametitle{Aplicaciones de $k$-means}

¿En cuáles de estas aplicaciones el método de $k$-means puede resultar apropiado?

\begin{enumerate}
\item Clasificar especies animales según características (peso, largo, longitud de la cola, ...).
\item Identificar distintas componentes de una imagen.
\end{enumerate}
...
\begin{minipage}[t]{.4\textwidth}
    ...
    \includegraphics[scale=0.2]{clase15-cluster-animales.jpg}
    \end{center}
\end{minipage}%
\begin{minipage}[t]{.6\textwidth}
    ...
    \includegraphics[scale=0.35]{clase15-imageSegmentation.png}
    \end{center}
\end{minipage}
...
\begin{frame}
\frametitle{$K$-medias como problema de optimización}
...
\begin{minipage}{.3\textwidth}
    \begin{center}
    \includegraphics[scale=0.6]{clase15-wcss.png}
    \end{center}
\end{minipage}

\begin{itemize}
\item Buscamos minimizar la "suma de los errores".
\item Consideramos que los errores son las distancias (al cuadrado) de cada punto al centro de su cluster: $\left\|\mathbf {x} -{\boldsymbol {\mu }}_{i}\right\|^{2}$.
\item Es decir, queremos minimizar la suma de los cuadrados dentro de cada cluster (WCSS o within cluster sum of squares).
\item Formalmente queremos elegir clusters $S_1, \dots, S_k$ y centros ${\boldsymbol {\mu }}_1, \dots, {\boldsymbol {\mu }}_k$ que minimicen:
$$
%\mathop {\operatorname {arg\,min} } _{\mathbf {S} }
WCSS = \sum _{i=1}^{k}\sum _{\mathbf {x} \in S_{i}}\left\|\mathbf {x} -{\boldsymbol {\mu }}_{i}\right\|^{2}
%(\{(S_1, {\boldsymbol {\mu }}_1), \dots, (S_k, {\boldsymbol {\mu }}_k)\})
$$
\end{itemize}
...
\begin{frame}
\frametitle{Ventajas y Desventajas del algoritmo $k$-medias naive}
\begin{columns}
    \column{0.5\textwidth}
    \textbf{Ventajas:}
    \begin{itemize}
...
\begin{frame}
\frametitle{¿Número óptimo de clusters?}

\begin{itemize}
\item A medida que aumentamos la cantidad de clusters, el error WCSS disminuye.
\item Por lo tanto no hay un número óptimo de clusters.
\item Podemos mirar la curva y ver cuándo aumentar la cantidad de clusters no reduce significativamente el error WCSS.
\item Este proceso suele llamarse ``método del codo''.
\end{itemize}

\begin{minipage}{.5\textwidth}
    \begin{center}
    \includegraphics[scale=0.27]{clase15-WSS-1.png}
    \end{center}
\end{minipage}%
\begin{minipage}{.5\textwidth}
    \begin{center}
    \includegraphics[scale=0.27]{clase15-WSS-2.png}
    \end{center}
\end{minipage}
...
\begin{frame}
\frametitle{Requerimientos para poder aplicar $k$-medias}

\begin{minipage}{.4\textwidth}
$k$-medias supone las siguientes propiedades de los datos, y puede fallar si estas propiedades no se cumplen:

\begin{enumerate}
\item Los clusters son esféricos e isotrópicos (el mismo radio en todas las direcciones).
\item Los clusters tienen la misma varianza.
\item Todos los clusters tienen aproximadamente la misma cantidad de puntos.
\end{enumerate}
\end{minipage}%
\begin{minipage}{.6\textwidth}
    \begin{center}
    \includegraphics[scale=0.25]{clase15-unexpected.png}
    \end{center}
\end{minipage}
