...
Podemos plantear el problema de clustering por $k$-medias como un problema de optimización (y el algoritmo $k$-medias naive es una método simple de optimización).

\begin{itemize}
\item Buscamos minimizar la "suma de los errores".
\item Consideramos que los errores son las distancias de cada punto al centro de su cluster.
...
\begin{itemize}
\item A medida que aumentamos la cantidad de clusters, el error WSS disminuye.
\item Por lo tanto no hay un número óptimo de clusters.
\item Podemos mirar la curva y ver cuándo aumentar la cantidad de clusters no reduce significativamente el error WSS.
