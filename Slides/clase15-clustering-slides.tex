...
Existen diversos algoritmos de clustering. El algoritmo $k$-medias comenzó a utilizarse en los alrededor de 1960.
...
\begin{frame}
\frametitle{Algoritmo $k$-means (ingenuo)}

Comenzamos con un conjunto arbitrario de centros (por ejemplo $k$ puntos al azar).

Ahora repetimos los siguientes dos pasos hasta "alcanzar convergencia":
\begin{enumerate}
\item Actualizamos las etiquetas: para cada punto, buscamos cuál es el centro más cercano a ese punto y lo asignamos a ese cluster.
...
Podemos plantear el problema de clustering por $k$-medias como un problema de optimización.
...
\item Formalmente queremos elegir centros $S_i$ y etiquetas $\mu_i$ que minimicen:
$$
\mathop {\operatorname {arg\,min} } _{\mathbf {S} }\sum _{i=1}^{k}\sum _{\mathbf {x} \in S_{i}}\left\|\mathbf {x} -{\boldsymbol {\mu }}_{i}\right\|^{2}
$$
\end{itemize}
