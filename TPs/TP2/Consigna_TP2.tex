...
\item Las filas que comienzan con "\%" representan porcentajes y en algunas casillas el valor es \lstinline{#DIV/0!}. Reemplazar estos valores por valores apropiados o eliminar estas columnas.
\item Al finalizar la limpieza de datos, resetear los índices.
\item Guardar en un DataFrame \lstinline{X} solo las variables númericas, a partir de la columna 12 (\lstinline{Ast/90}). Para clustering y clasificación no vamos a utilizar las variables categóricas ni edad ni minutos jugados.
\end{enumerate}
...
\item Construir una base de datos $X$ con las variables explicativas (igual que en la sección de clustering) e $y$ con las variable de posición.
...
\end{enumerate}
\item Repetir el mismo método de clasificación con el dataset de jugadoras de la liga femenina. ¿Qué porcentaje de aciertos obtienen en este caso? ¿Puede modificarse el parámetro para obtener un porcentaje mayor?
\end{enumerate}
...
\begin{enumerate}
\item En 2021 Messi fue transferido del Barcelona al PSG. Si Barcelona quiere reemplazar a Messi por un jugador de características similares pero menor valor, qué jugador le recomendarían?
\item Queremos detectar jugadores ``baratos'', es decir cuya valuación en el mercado sea inferior a que la que nosotros estimemos. Para esto, construir primero un modelo para ajustar el valor de mercado a partir de los datos de los jugadores. Pueden utilizar redes neuronales o cualquier otro modelo.
\item El PSG quiere vender a Mbappe y reemplazarlo por otro jugador más barato. Hacer un listado de los 10 jugadores más parecidos a Mbappé según el criterio que elijan. De esos 10 jugadores, hay alguna ganga? Es decir, algún jugador que este valuado por abajo de las predicciones que hicieron en el punto anterior?
\end{enumerate}
\end{enumerate}
