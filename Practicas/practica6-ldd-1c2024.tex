...
\item Hallar un escalamiento lineal $f(x)$ que transforme los valores en el intervalo $[2,5]$ a valores en el intevalo $[0,1]$.
...
\begin{tikzpicture}
  ...
  \draw[blue, thick] (0,0) node[below] {$0$} -- (2,0) node[below] {$1$};
  % Interval of length 1
  \draw[red, thick] (4,1)  node[above] {$2$} -- (10,1) node[above] {$5$};
  % Mapping arrow
  \draw[->, dashed] (4,1) -- (0,0) node[midway, left] {$f$};
  \draw[->, dashed] (10,1) -- (2,0) node[midway, left] {$f$};
\end{tikzpicture}

\item Para este ejercicio utilizar los datos \lstinline{CollegeBasketballPlayers2009-2021.csv}.
\begin{enumerate}
\item Ajustar un modelo de regresión Ridge para predecir los puntos promedio (\lstinline{pts}) en función de las demás variables. 
Para ellos, escalar previamente las variables utilizando escalamiento MinMax.
\item A partir de los resultados, según los coeficientes obtenidos, ¿cuáles son las 5 variables de mayor peso en el modelo?
\end{enumerate}
